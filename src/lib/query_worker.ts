...
import { type SafePromise, type Result, error, result, isError } from "result-interface";
    ...
    case "query": {
      ...
      if (isError(rulesResponse)) {
        response = <WorkerErrorResponse>{
          type: "error",
          result: `the rules are malformed: ${rulesResponse.error}`,
        };
        break;
      }
      const {
        value: { rules, disallowedRules, rulesKg },
      } = rulesResponse;
      const tracker: ITrackedInfo = { links: [], schemaAlignment: [] };
      const bindingStreamResult = await executeQuery(
        query,
        rules,
        disallowedRules,
        schemaAlignment,
      ...
      const resultReport = await reportResults(
        bindingStreamResult,
        self.postMessage,
        rulesKg,
        tracker
      ...
      if (isError(resultReport)) {
        response = <WorkerErrorResponse>{
          type: "error",
          result: `there was an error in the query: ${resultReport.error}`,
        };
        break;
      }
...
async function convertRules(
  ...
  { rules: Map<string, RDF.Quad[]>; disallowedRules: string[]; rulesKg: RDF.Quad[] },
  string
> {
...
function reportResults(
  ...
  postFunction: (message: WorkerBindingResponse | WorkerErrorResponse) => void,
  rulesKg: RDF.Quad[],
  tracker: ITrackedInfo
): SafePromise<IReportedResults, string> {
...
function ruleSetToKg(ruleSet: IRuleSet, writer: N3Writer): void {
  ...
  const declaration = <RDF.Quad>(
    DF.quad(
      ruleSetNode,
      DF.namedNode("http://www.w3.org/1999/02/22-rdf-syntax-ns#type"),
      DF.namedNode(`${SEM_MAP_PREFIX}RuleSet`)
    )
  );
  const subwebQuad = <RDF.Quad>(
    DF.quad(ruleSetNode, DF.namedNode(`${SEM_MAP_PREFIX}subweb`), DF.literal(ruleSet.subweb))
  );

  const kg: RDF.Quad[] = [declaration, subwebQuad];

  for (const rule of ruleSet.rules) {
    const node = DF.blankNode();
    const ruleDeclation = <RDF.Quad>(
      DF.quad(ruleSetNode, DF.namedNode(`${SEM_MAP_PREFIX}rule`), node)
    );
