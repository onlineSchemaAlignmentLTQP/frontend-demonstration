<script lang="ts">
  ...
  import queriesMultipleVocab from "../../example_queries/multipleVocab.json?raw";
  import { EVENT_TARGET, PROPOSED_QUERY_EVENT, CHANGE_RULE_EVENT } from "../state.svelte";

  let isOpen = $state(false);

  const queries = JSON.parse(queriesMultipleVocab);

  function changeQuery(description:string){
    const queryEvent = new CustomEvent(PROPOSED_QUERY_EVENT, {"detail":queries[description]["query"]})
